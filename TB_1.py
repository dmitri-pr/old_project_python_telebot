...
def users_base_search(cid, *args):
    ...
    # rows = cur.fetchall()
    rows = DB.check_users_ids_without_self_id(cid)
    l = [arg for arg in args if arg != 'Неважно']
    if l == []:
        if [row[1] for row in rows if row[1] != cid and user_step.get(row[1], 0) != 10
                                      and user_step.get(row[1], None) != None and row[1] not in user_pairs.keys() and
                                      row[1] not in
                                      user_pairs.values()] == []:
            result = None
        else:
            result = random.choice([row[1] for row in rows if row[1] != cid and user_step.get(row[1], 0) != 10
                                    and user_step.get(row[1], None) != None and row[1] not in user_pairs.keys() and row[
                                        1] not in
                                    user_pairs.values()])
    else:
        if [row[1] for row in rows if all(arg in row for arg in l
                                          ) and row[1] != cid and user_step.get(row[1], None) != 10 and user_step.get(
            row[1], None) != None
                                      and row[1] not in user_pairs.keys() and row[1] not in user_pairs.values()] == []:
            result = None
        else:
            result = random.choice([row[1] for row in rows if all(arg in row for arg in l
                                                                  ) and row[1] != cid and user_step.get(row[1],
                                                                                                        0) != 10 and user_step.get(
                row[1], None) != None
                                    and row[1] not in user_pairs.keys() and row[1] not in user_pairs.values()])
    return result
...
def enter_filter_field_or_return_back(text, markup_):
    ...
    def callback_query(call):
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text=text, reply_markup=markup_())
        user_step[call.message.chat.id] = 0


def change_filter_field(text, index, text_, markup_set_field_search):
    @bot.callback_query_handler(func=lambda call: call.data == text)
    def callback_query(call):
        user_filter[call.message.chat.id][index] = text_
        text = f'Ваш фильтр:\n👫Пол: {user_filter[call.message.chat.id][0]}\n🗓️Возраст: \
        {user_filter[call.message.chat.id][1]}\n🏙️Город: {user_filter[call.message.chat.id][2]}'
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
...
def constant_part_of_query(callmessagechatid, callmessagemessage_id, markup_):
    text = f'Ваш фильтр:\n👫Пол: {user_filter[callmessagechatid][0]}\n🗓️Возраст: \
    {user_filter[callmessagechatid][1]}\n🏙️Город: {user_filter[callmessagechatid][2]}'
    bot.edit_message_text(chat_id=callmessagechatid, message_id=callmessagemessage_id,
                          text=text, reply_markup=markup_())


def rating_with_like_dislike(callid, like, cid, index, mid):
    bot.answer_callback_query(callid, like)
    for k, v in user_pairs.items():
        if cid == k:
            users_likes[v][index] += 1
...
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('Настроить поиск ⚙️', markup_set_search)
...
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('Пол 👫', markup_set_sex_search)
...
#                                    text=text, reply_markup=markup_set_sex_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('Мужской \u2642\uFE0F', 0, 'Мужской \u2642\uFE0F', markup_set_sex_search)
...
#                                    text=text, reply_markup=markup_set_sex_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('Женский \u2640\uFE0F', 0, 'Женский \u2640\uFE0F', markup_set_sex_search)
...
#                                    text=text, reply_markup=markup_set_sex_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('None_sex', 0, 'Неважно', markup_set_sex_search)
...
#                                    text=text, reply_markup=markup_set_search())
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('back_from_set_sex_to_set_search', markup_set_search)
...
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('Возраст 🗓️', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('14', 1, 'до 14 лет', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('17', 1, '15-17 лет', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('21', 1, '18-21 год', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('25', 1, '22-25 лет', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('35', 1, '26-35 лет', markup_set_age_search)
...
#                                     text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('35+', 1, '35+ лет', markup_set_age_search)
...
#                                    text=text, reply_markup=markup_set_age_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('None_age', 1, 'Неважно', markup_set_age_search)
...
#                                    text=text, reply_markup=markup_set_search())
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('back_from_set_age_to_set_search', markup_set_search)
...
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('Город 🏙️', markup_set_town_search)
...
def callback_query(call):
    ...
    # bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
    #                                 text=text, reply_markup=markup_set_town_search())
    constant_part_of_query(call.message.chat.id, call.message.message_id, markup_set_town_search)
    user_step[call.message.chat.id] = 0
...
def callback_query(call):
    ...
    constant_part_of_query(call.message.chat.id, call.message.message_id, markup_alphabet)
...
#                                    text=text, reply_markup=markup_set_town_search())
#     user_step[call.message.chat.id] = 0
change_filter_field('None_town', 2, 'Неважно', markup_set_town_search)
...
#                                    text=text, reply_markup=markup_set_search())
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('back_from_set_town_to_set_search', markup_set_search)
...
#     user_step[call.message.chat.id] = 0
enter_filter_field_or_return_back('back_from_set_to_search', markup_search)
...
def callback_query(call):
    ...
    # bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
    #                                text=text, reply_markup=markup_search())
    constant_part_of_query(call.message.chat.id, call.message.message_id, markup_search)
    user_step[call.message.chat.id] = 0
